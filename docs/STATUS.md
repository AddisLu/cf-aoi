...
| ★2 | 多相機+背壓 CRC 損毀 | **✅ 已根治並驗證**（改 SEND/RECV） | 見下方專節 |
...
#### ★2 專節：已根治 — 資料路徑 `RDMA_WRITE_WITH_IMM` → `SEND/RECV`

**最終驗證（同一組最嚴苛條件，2 台相機 + 背壓 `--max-queue-size 2 --test-consumer-delay-ms 1200`）**：

| `rdma_slots` | 舊 `WRITE_WITH_IMM` | 新 `SEND/RECV` |
|---|---|---|
| 2 | err=**11**/20 | **ok=20 err=0** ✅ |
| 4（預設）| err=**10**/20 | **ok=20 err=0** ✅ |
| 16 | err=0/20 | **ok=20 err=0** ✅ |

三種深度背壓皆確認生效（`push≈997ms`/幀、吞吐由消費端節奏決定 0.62 / 0.88 / 3.31 fps）。
**ring 深度現在只影響吞吐（pipeline 深度），不再影響正確性** —— 這才是應有的行為。

**修法（為何是 by construction 正確，不是縮小窗口）**：`SEND` 的資料落點由**收端的 recv WQE** 決定，
送端不再指定遠端位址。IP 端 N 個 `post_recv` 各指向一個 slot（`wr_id` = slot 編號），
**處理完該 slot 才重掛它的 WQE**；沒有 WQE 時 RNR 發生在「放置資料之前」→
slot 不可能在 IP 讀取期間被覆寫。舊 `WRITE` 由送端自算位址，RNR 只擋 immediate、payload 照樣落地，
且 RNR 期間持續重試、每次重寫該 slot ⇒ 縮短收端讀取窗口只能降低命中率，消不掉（見下方失敗紀錄）。

配套改動：收端 `slot_id` 改由 `wc.wr_id` 取得（不再 `seq % n_slots` 推算）；`seq` 改從 payload 內
`FrameHeader.frameSeq` 取（SEND 無 imm，且 uint64 不再受 imm 的 32-bit 截斷）；新增 `wc.byte_len`
與 header 宣告長度對帳（WRITE 路徑拿不到此資訊）。背壓鏈行為不變。
⚠️ `rdma_common.h` 有 `grab/src/` 與 `ip/src/image_source/` **兩份同源副本**，`post_recv` 的 `wr_id`
參數兩份都要加（只改一份會編不過／行為分歧）。

**完整回歸（SEND 版）**：`verify_step3_trigger`（2 台）**6/6 PASS**；連續模式 `GRAB_STOP` **OK 350ms**；
正常負載 20 幀 err=0；**最終整合：4 片 × 3 張 × 2 台 = 24 幀 → CRC 不符 0、seq 跳躍 0、
recv ok=24 err=0、24 個輸出夾全不同名、dropped=0**（`rdma-process` 真 GPU 檢測鏈）。

<details><summary>失敗紀錄：前兩次嘗試為何不夠（保留，避免日後重蹈）</summary>

1. **把 `++frame_seq` 移進 `send_mtx`**（假設是 seq 順序 ≠ wire 順序）→ **完全無效，仍 err=10/20**。
2. **收端 memcpy 上移、CRC 改對副本**（把持有期 885ms → 5ms）→ err 10→**7**，改善但仍損毀。
   證明「縮短收端讀取窗口」治不了：送端的 write 一旦 post 出去就持續 RNR 重試、每次重寫該 slot，
   是一場針對該 slot 的持續覆寫風暴，不是一次性競態。

**定位方法（值得複用）**：不改程式碼、只改 `--rdma-slots`（2/4/16 → err 11/10/0）做決定性實驗，
把原因鎖在「slot 重用」而非「相機台數」；再看「seq 1–4（初始 credit）全對、seq 5（第一個重用 slot 的幀）
開始壞」定位到重用邊界。
</details>
...
**優化鏈（160 幀 / 8 執行緒模擬多相機共用單一 QP / 8160×5000）**：

| 階段 | 含 app-CRC | 關 app-CRC | 說明 |
|---|---|---|---|
| 起點（單執行緒量測）| 39.2 | 90.3 | — |
...
—— 幾無差別，純 memcpy 只要 1.69ms。真正的 10ms 是 **vector 歸零 + 每幀新配置 40.8MB 的 page fault**。
② + ③ 合計約 15 行改動，效果 2.1×；原本規劃的零複製重構完全不需要。

**驗證**：背壓回歸（8 緒 + `slots=2` + 400ms 延遲 + 含 CRC）**ok=32 err=0**（push=358.6ms 確認背壓生效）；
`slots=4` + 300ms + 8 緒 **ok=40 err=0**。速度優化未犧牲 slot 安全性。
另修：`rdma-validate` 的二次 CRC 驗證未尊重 `CFAOI_RDMA_NOCRC=1`（送端該旗標下 crc32 填 0）
→ 每幀誤報，實測 `ok=0 err=160` 看似資料全壞、實為誤報。

**存圖策略（生產必要）**：overlay（8160×5000×3ch PNG）實測 **406–410 ms/幀** = GPU 檢測的 **56 倍**，
逐幀存上限僅 2.4 幀/s。`rdma-process` 已改為**只在有缺陷時存**（`SaveOptions.overlay_on_defect_only`，
...
| Grab（寫入端）| **L3 ✓（2026-07-30 damac 實機驗通）** | 見下方實測 |
...
**Grab 端實測（2026-07-30 damac，第一次編譯即通過，2 台真相機）**：

| 測項 | 結果 |
|---|---|
| 壞資料一律擋、原檔不動 | `cam_id` 重複／MAC 重複（不同格式，證正規化）／MAC 格式錯／空表／`entries` 非陣列 → **5/5 ERR**，訊息明確 |
| 正常寫入 | 把兩台**對調**（CCD00↔CCD01）→ `OK written=2`，`LIST_CAMERAS` **立即反映**（raL4096→CCD00、raL8192→CCD01）|
| 安全守門 | **已 ARM → ERR**、**取像中 → ERR**（皆回「請先 GRAB_STOP」）、**STOP 後放行 OK** — 三態全對 |
| 落檔正確性 | `.bak` 備份已產生；**重啟 cfaoi_grab 後新檔正確載回**（寫得進去也載得回來）|
| 回歸 | `verify_step3_trigger`（2 台）**6/6 PASS**；`rdma-process` 10 幀 **CRC/seq/遺失錯誤 0**，輸出夾 CCD00/CCD01 由 MAC 映射決定 |

⚠️ 第一次跑 `verify_step3_trigger` 出現 FAIL，查為**收端未重啟**（前一輪測試斷線後 cfaoi_ip 已退出），
非程式問題；重啟 IP 後 6/6 全 PASS。
...
**Step 3 剩餘（需相機陣列）——軟體已備妥（2026-07-21），到貨即測：**
1. **SN2201 交換器到貨** + 37 台 raL8192 接線
2. `cfaoi_grab --cam-count ALL`（cam_manager L1 已落地：平行 arm/N 張自動停/totalSlice 真值；待實機）
3. Control↔Grab 觸發鏈 L2 已落地（GRAB_ARM 預熱/GRAB_START ms 級觸發/CF_ 全接線）；
   **到貨日驗證：`scripts/verify_step3_trigger.py`（6 階段：ARM 耗時/觸發延遲<100ms/收滿對帳/
   第二片節拍/teardown）+ IP log `[EdgeCheck] camX slice0 前緣` 行號 spread = 37 台啟動 skew 實測**
4. ⚠ grab 完整編譯待 damac 開機（cam_manager/control_server 已語法驗證；pylon 依賴只在 damac）

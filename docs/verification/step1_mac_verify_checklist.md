...
| A4 | 左鍵在影像上拖曳（移動 >4px） | 影像平移、放開停住（觸控板單指 / 滑鼠左鍵皆可，**免鍵盤免中鍵**） | ☐ |
| A4b | 左鍵在空白處輕點一下（幾乎不動） | **不平移**（<4px 視為點選；命中缺陷才選取，見 B4） | ☐ |
| A4c | 中鍵拖曳 或 空白鍵+左鍵拖曳（備用路徑） | 同樣可平移（保留給習慣此操作者） | ☐ |
| A5 | 滑鼠移到影像上 | 狀態列 `Axis : (x,y)` 顯示原始像素座標、`Value : v` 顯示該點灰階值；移出影像顯示 `-` | ☐ |
| A6 | 按住 `M` 鍵，左鍵點兩點 | 兩紅圈+連線；狀態列 `Region` 顯示 `dx= dy= d=`（量 Pitch 用）；點第三點會清空重量 | ☐ |

> 全 8 項過 → STATUS「影像 Viewer 互動」**L1 → L3**。（A4 為 2026-06-18 新平移方案：左鍵拖曳平移 + 輕點選缺陷，觸控板/鍵鼠通用）

...
| **F. 真 recipe 全鏈重驗** | ✅ **完成** | IP04 真 RecipeInfo.xml(SUB+remap+smooth+全panel ROI Y→146k,X全寬)+ 全 31 slice 拼接(8160×155000)。**027→X5894 Y135725 Size11 PointDark✓、028→X1853 Y142371 Size4 PointDark✓**(= per-slice 偵測同一缺陷)。全 panel 共 46 缺陷(sizes 1-11,合理非洪水)、GPU 1791ms。**驗收完整達標。** 100G:damac→spark `auo001@192.168.3.1` scp(經 Addis 授權設金鑰),1.2GB/3.2s。 |
| **E. Blob 過濾** | ⏳ 選做(非阻塞) | BlobMinSize=3/MergeDistance=26 精修。**開放問題已解:全panel 下 028 缺陷 Size=4 ≥3 → 不會被濾(先前 Size=1 是錯 ROI/per-slice 假象)。** Blob 可進一步合併/濾 size<3 的零星點;需 legacy golden 才能斷哪些 size 1-2 是真/FP。 |

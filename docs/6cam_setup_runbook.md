...
| 交換機埠 | SN | MAC | CCD | IP |
|---|---|---|---|---|
| WGE1/0/37 | 25564093 | 00:30:53:54:E6:BD | CCD04 | 192.168.5.4 |
| WGE1/0/39 | 25563179 | 00:30:53:54:E3:2B | CCD03 | 192.168.5.3 |
| WGE1/0/41 | 25563177 | 00:30:53:54:E3:29 | CCD02 | 192.168.5.2 |
| WGE1/0/43 | 25563161 | 00:30:53:54:E3:19 | CCD01 | 192.168.5.1 |

（2026-09-18 定案：編號 1 開頭、IP 尾碼 = 編號。埠序與編號相反是實體配置使然，以相機內的名稱為準。）

- [x] **4 台 IP 已對齊規則**（2026-09-21）：改名時 IP 沒跟著動（.1↔.4、.2↔.3 反向），
      當天用暫時位址繞開撞號改回來——`set 25563161 CCD01 192.168.5.101`（讓出 .4）→ CCD04 進 .4 →
      CCD01 回 .1 → 同法處理 .2/.3。**換號時的兩個坑**：
      ① `cam_provision set` 要求相機未被佔用，**pylon Viewer 開著就會 `controlled by another application`**（先關）；
      ② 換完 `ping` 會短暫不通——本機 ARP 還記著舊的 MAC↔IP 配對，等它自己重解即可（別急著改網路設定）。
- [ ] persistent IP 斷電複驗（寫進 flash，但尚未實測關機再開是否留存）。

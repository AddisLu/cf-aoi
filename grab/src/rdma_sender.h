...
public:
    ...
    // app-CRC 計算。**必須在 send_mtx 之外呼叫**，算完再把結果傳給 send_frame——
    // CRC 是整個送端最貴的一步（生產幀 40.8MB 實測 ~16ms），留在鎖內會讓 37 台全部序列化。
    // CFAOI_RDMA_NOCRC=1 → 回 0（跳過）。
    static uint32_t crc_of(const uint8_t* payload, uint32_t payload_bytes);
    ...
    // payload_crc32：呼叫端用 crc_of() 先算好（見上）。

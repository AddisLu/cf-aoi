...
11. **行速率必須由 grab 顯式設定，不可繼承相機 flash（2026-09-21）**：
   ...
   - ⚠️ **生產是 free-run + 軟體觸發，不是 encoder 行觸發**（2026-09-21 Addis 確認產線做法：
     Sheet Panel 進片碰 sensor → 上位機下 CF_GRAB_START → Control → Grab `GRAB_START` →
     所有相機軟體觸發開始取像 → 收滿 `frames_per_panel` 張自動停）。
   - **因此行速率決定的是「影像比例尺」，不是快慢**：無 encoder ⇒
     `Y 解析度(µm/line) = 產線速度(µm/s) ÷ 行速率(Hz)`，
     `每張涵蓋長度 = 5000 × Y 解析度`。
     **`--line-rate max` 只適合 bench／不讓設定靜默繼承 flash，生產必須顯式指定**：
     `--line-rate = 產線速度(µm/s) ÷ 目標 Y 解析度(µm/line)`
     （例：96mm/s、8µm/line → **12,000 Hz**，此時每張 = 40.0mm，與 `ip/src/edge_check.h` 的
     40mm 窗口、`expected_panel_lines = 玻璃長/行解析度` 一致）。
   - **同批相機行速率不一致 = 影像幾何錯誤，不只是吞吐差**：本次 11,001 vs 12,195（96mm/s 下
     8.73 vs 7.87 µm/line）會讓同一缺陷在掃描方向的量測值差 **11%**，且各台每張涵蓋 43.6 vs
     39.4mm → 固定張數下掃到的面板範圍不同。單看 fps 永遠看不出來。
   - **產線速度上限**：行速率天花板 12,195 Hz ⇒ 8µm/line 時最快支援 **97.6 mm/s**；
     要更快只能放寬 Y 解析度。
